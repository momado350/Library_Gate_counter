...
detector.setPreferableBackend(cv2.dnn.DNN_TARGET_MYRIAD)
    ...
    while True:
        ...
        for (objectId, bbox) in objects.items():
            ...
            if objectId not in object_id_list:
                ...
                with open('Attendance.csv', 'r+') as f:
                    if objectId > 0:
                        objectId = random.randrange(1000, 9000, 3)
                        myDataList = f.readlines()
                        nameList = []

                        nameList.append(objectId)
